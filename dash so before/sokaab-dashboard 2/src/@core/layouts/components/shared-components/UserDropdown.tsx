...
const UserDropdown = (props: Props) => {
  let data: any = window.localStorage.getItem('userData')
  if (data) {
      ...
      >
        <Box sx={{ py: 1.75, px: 6 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            >
              <Avatar
                // alt='John Doe' src='/images/avatars/1.png'
                sx={{ width: '2.5rem', height: '2.5rem' }}
              />
            </Badge>
            <Box sx={{ display: 'flex', ml: 2.5, alignItems: 'flex-start', flexDirection: 'column' }}>
              {/* <Typography sx={{ fontWeight: 500 }}>Abokor hassan</Typography> */}

              {data && data.fullName && (
                <Typography sx={{ fontWeight: 500 }}> {data.fullName || 'John Doe'} </Typography>
              )}
