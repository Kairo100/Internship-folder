    ...
    UserModel.findOne({ email })
        .then(user => {
            ...
            bcrypt.compare(password, user.password, (err, match) => {
                ...
                if (match) {
                    const token = jwt.sign({ email: user.email, username: user.username }, process.env.JWT_SECRET_KEY || "jwt-secret-key-fallback", { expiresIn: '3d' }); // Add expiration
...
app.post('/create', verifyUser, async (req, res) => {
    try {
        ...
        const newPost = new PostModel({
            title,
            description,
            file, // <- This can be a full image URL, like from Cloudinary or another source
            email: postEmail,
        });
