...
export class StatisticsController {
  ...
  @Get('dashboard')
  @Public()
  async getDashboardStatistics(@Req() request) {
    ...
    if (
      ...
      whereClause.organisation_id = loggedInUser.organisation_id;
    }
...
//org stat
  @Get('organisations/:id/metrics')
  @Public() // You might want to remove this and add a Guard for authentication
  async getOrganizationMetrics(@Param('id') organizationId: number) {
    ...
    // Check if the organization exists and has projects
    if (projects.length === 0) {
      // You can return a 404 or an empty set of metrics
      return {
        total_projects: 0,
        active_projects: 0,
        total_funding_raised: 0,
        total_matching_funding: 0,
      };
    }
  ...
  (project) => project.status?.toLowerCase() === 'Live'.toLowerCase(),
    ...
    // Get all account numbers for projects within this organization
    ...
    if (projectsAccountNumbers.length > 0) {
      ...
      total_funding_raised = transactions.reduce((sum, transaction: any) => {
        return transaction.DrCr.toLowerCase() === 'cr'.toLowerCase()
          ? sum + parseFloat(transaction.TranAmt)
          : sum;
      }, 0);
    ...
    // Calculate total matching funds from the fetched projects
    total_matching_funding = projects.reduce((sum, project) => {
      return sum + Number(project.available_grant);
    }, 0);
